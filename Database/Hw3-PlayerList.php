...
print'<div style="margin: 15px 20px 25px 225px; font-size:25px">';
// 如果有資料
if ($result) {
...
if(!empty($result)){
    ...
    print '<td>'."球員生日".'</td>';
    print '<td>'."球員性別".'</td>';
    print '<td>'."右(左)投/右(左)打".'</td>';
    print '<td>'."球員身高".'</td>';
    print '<td>'."球員體重".'</td>';
    print '<td>'."球員國家".'</td>';
    print '<td>'."球員隊伍".'</td>';
    print '<td>'."加盟日期".'</td>';
    print '<td>'."現役/非現役(解約日期)".'</td>';
    print '</tr>';
    foreach($datas as $key => $value){
        print '<tr>';
        print '<td>'.$value[0].'</td>';
        print '<td>'.'<a href="Hw3-PlayerInformation.php?id='.$value[3].'">'.$value[3].'</a>'.'</td>';
        print '<td>'.$value[2].'</td>';
        print '<td>'.$value[1].'</td>';
        print '<td>'.$value[4].'</td>';
        print '<td>'.$value[5].'</td>';
