		...
		while (getline(ifsObj, tmp_str)) {		// read line
			if (tmp_str.size() == 0);
				...
				if (spilt_str[0] == "mtllib") {
					...
					//string mtl_filepath = "models/" + mtl_filename[0] + "/" + mtl_filename[0] + ".mtl";
							...
							if (mtl_value.size() == 0)
								...
								if (mtl_value[0] == "newmtl") {
									...
									while (true) {
										...
										if (mtl_value.size() == 0 || ifsMtl.eof() == true ) {
...
void TriangleMesh::Draw(const SubMesh &submesh_Obj)
{
	...
	glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(VertexPTN), (const GLvoid*)20);
