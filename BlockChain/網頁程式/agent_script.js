...
web3.eth.defaultAccount = '0x1dc531F45c42DeAf77e1C09946bdDAb477448127';
...
function list_product(){
	return new Promise((resolve,reject)=>{
		ResumeContract.methods.List_All_Processed_Id().call(function(error, result)
		{
			if(!error)
			{
				console.log(result);
...
function list_data(num){
	return new Promise((resolve,reject)=>{
		ResumeContract.methods.List_Processed_Data(Processed_Id[num]).call(function(error, supplied)
		{
			if(!error) 
			{
...
async function Start(){
	console.log("start");
	await list_product();
	for(var count = 0; count < Processed_Id.length; count++) await list_data(count);
	build_box();
	console.log("done");
}
			...
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body">
						<form>
							<div class="form-group">
								...
								<input type="text" class="form-control ingredient" style="margin-top:13px" placeholder="輸入價格" id="price${i}"/>
								<input type="text" class="form-control ingredient" style="margin-top:13px"   placeholder="輸入提供的售後服務" id="service${i}"/>
								<input type="text" class="form-control ingredient" style="margin-top:13px"   placeholder="輸入包裝方法" id="method${i}"/>
					...
					<div class="modal-footer">
						<button type="button" id=${i} class="btn btn-info" onclick="handle_click(this)" data-bs-dismiss="modal">包裝</button>
...
function handle_click(element){
	...
	ResumeContract.methods.Agent_Work(data[element.id].dataid, $("#name"+element.id).val(), $("#method"+element.id).val(), $("#service"+element.id).val(), $("#price"+element.id).val()).estimateGas({from : web3.eth.defaultAccount}, function(error, result){
        if(!error){
            console.log(result);
            ResumeContract.methods.Agent_Work(data[element.id].dataid, $("#name"+element.id).val(), $("#method"+element.id).val(), $("#service"+element.id).val(), $("#price"+element.id).val()).send({from : web3.eth.defaultAccount, gas: result}, function(error2, result2){
